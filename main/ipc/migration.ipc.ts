...
export function createMigrationIpc(storage: ShaderStorage, markerPath: string) {
  return defineIpcModule('migration', {
    ...
    decline: handle(async () => { await writeFile(markerPath, 'declined\n', 'utf8'); }),
    select: handle(async (event): Promise<MigrationResult> => {
      const owner = BrowserWindow.fromWebContents(event.sender) ?? undefined;
      const options: OpenDialogOptions = { properties: ['openDirectory'], title: 'Choose an existing Shader Studio data folder' };
      ...
      try {
        const dataDir = basename(selected).toLowerCase() === 'shaders' ? dirname(selected) : selected;
        if (!(await exists(join(dataDir, 'shaders')))) {
          return { status: 'error', message: 'The selected folder does not contain a shaders directory' };
        }
        const source = new ShaderStorage({ dataDir, examplesDir: join(dataDir, '__no_examples__'), seed: false });
        await source.init();
        const ids = await source.listIds();
        const payloads = [];
        let skipped = 0;
        for (const id of ids) {
          try { payloads.push(await source.exportOne(id)); } catch { skipped += 1; }
        }
        const result = await storage.importPayloads(payloads, 'overwrite');
        await writeFile(markerPath, JSON.stringify({ importedAt: new Date().toISOString(), source: dataDir }), 'utf8');
