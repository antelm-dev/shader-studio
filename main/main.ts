...
const scheme = createCustomScheme(env.scheme, { standard: true, secure: true, supportFetchAPI: true });
...
async function serveClient(request: Request): Promise<Response> {
  ...
  try { requested = decodeURIComponent(new URL(request.url).pathname.replace(/^\/+/, '')); }
  ...
  try {
    ...
    return new Response(data, { headers: { 'content-type': MIME_TYPES[extname(file)] ?? 'application/octet-stream' } });
  } catch {
...
interface WindowState { bounds?: Electron.Rectangle; maximized?: boolean }

function validBounds(value: unknown): Electron.Rectangle | undefined {
  const item = value as Partial<Electron.Rectangle> | null;
  if (!item || !['x', 'y', 'width', 'height'].every((key) => Number.isFinite(item[key as keyof Electron.Rectangle]))) return undefined;
...
async function readWindowState(path: string): Promise<WindowState> {
  try { return JSON.parse(await readFile(path, 'utf8')) as WindowState; } catch { return {}; }
}

const closeController: CloseController = { approved: new WeakSet() };
let mainWindow: BrowserWindow | null = null;

if (!app.requestSingleInstanceLock()) app.quit();
else app.on('second-instance', () => { mainWindow?.restore(); mainWindow?.focus(); });
...
prepare({
  ...
  onReady: async () => {
    ...
    const examplesDir = env.production ? join(process.resourcesPath, 'examples') : resolve('examples');
    const storage = new ShaderStorage({ dataDir: join(userData, 'library'), examplesDir });
    await storage.init();

    const ipc = createIpcContainer();
    await ipc.loadAll({
      shader: createShaderIpc(storage), files: createFilesIpc(),
      migration: createMigrationIpc(storage, migrationPath), window: createWindowIpc(closeController),
    });

    const win = new BrowserWindow({
      width: bounds?.width ?? 1440, height: bounds?.height ?? 900, x: bounds?.x, y: bounds?.y,
      minWidth: 800, minHeight: 600, show: false, frame: false, backgroundColor: '#090b10', icon: env.paths.icon,
      webPreferences: { preload: env.paths.preload, sandbox: true, contextIsolation: true, nodeIntegration: false },
    });
    mainWindow = win;
    ...
    const pushWindowState = () => {
      win.webContents.send('state-changed', { maximized: win.isMaximized(), fullscreen: win.isFullScreen() });
    };
    const saveState = () => void writeFile(statePath, JSON.stringify({ bounds: win.getNormalBounds(), maximized: win.isMaximized() }), 'utf8');
    win.on('resize', saveState); win.on('move', saveState);
    win.on('maximize', () => { saveState(); pushWindowState(); });
    win.on('unmaximize', () => { saveState(); pushWindowState(); });
    ...
    win.webContents.on('will-navigate', (event, url) => {
      const allowed = env.production ? url.startsWith(`${env.scheme}://`) : url.startsWith(env.devServerUrl);
