...
describe('parseBundle', () => {
  ...
  it('round-trips a real multi-pass project unchanged', () => {
    const project = addBuffer(migrateLegacyProject(FRAGMENT, VERTEX));
    const bundle = buildShaderBundle(payload({ project }));

    const parsed = parseBundle(bundle);
    expect(parsed.ok).toBe(true);
    expect(parsed.ok === true && parsed.value[0].project).toEqual(project);
  });

  it('sanitizes a malformed project rather than rejecting the whole import', () => {
    const bundle = buildShaderBundle(payload({ project: { garbage: true } as never }));

    const parsed = parseBundle(bundle);
    expect(parsed.ok).toBe(true);
    const project = parsed.ok === true ? parsed.value[0].project : null;
    expect(project && imagePass(project).source).toBe(FRAGMENT);
  });

  it('accepts a shader-studio/v1 bundle, synthesizing a project via migrateLegacyProject', () => {
    // A v1 bundle predates the `project` field entirely.
    const v1Shader: Partial<ShaderPayload> = payload();
    ...
    // Structural, not a deep-equal against a second `migrateLegacyProject`
    // call: ids are generated fresh each call and would never match.
    const project = parsed.ok === true ? parsed.value[0].project : null;
    expect(project?.vertex).toBe(VERTEX);
    expect(project?.files).toEqual([]);
    expect(project && imagePass(project).source).toBe(FRAGMENT);
    expect(project && imagePass(project).channels).toEqual([
      { kind: 'texture', slot: 0 },
      { kind: 'texture', slot: 1 },
      { kind: 'texture', slot: 2 },
      { kind: 'texture', slot: 3 },
