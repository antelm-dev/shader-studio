...
// --- C4 -------------------------------------------------------------------
// The slice of `account/account-session.ts` (Task 03) this service consumes.
// At integration, replace these two declarations with a type import from there.

export interface AccountState {
  status: 'disabled' | 'signed-out' | 'signed-in' | 'reauth-required';
...
export interface AccountSession {
  ...
  /** Fetch against the account server with the bearer token; marks `reauth-required` on 401. */
